...
test('existing saves immediately receive the shorter ocean target without losing progress',()=>{
 const game=createGame();game.cash=2000000;game.litres=80000;
 const restored=restoreGame(JSON.stringify(game));
 assert.equal(restored.won,true);assert.equal(temperature(restored),100);assert.equal(restored.cash,2000000);
 game.litres=40000;const halfway=restoreGame(JSON.stringify(game));assert.equal(halfway.won,false);assert.equal(temperature(halfway),59);
});
